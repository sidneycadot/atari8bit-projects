                ...
                ;
                ; The code size is currently a modest 38 bytes.
                ;
...
loop8:          sec                 ; [2] Burn 8 cycles if A != 1 at the start of the loop, else 6 cycles.
                sbc     #1          ; [2]
                bne     loop8       ; [Z=0: 4, Z=1: 2]  *** IT IS CRITICAL THAT THIS BRANCH CROSSES A PAGE BOUNDARY ***
...
long_delay:     ; This is the code path taken if a delay count of >= 256 cycles is requested.
                ;
                ; The approach to accomplish the desired behavior is to decrease the number of delay cycles
                ;   requested by 9 in a loop that takes precisely 9 cycles, until X becomes zero, then
                ;   rejoin the 'short_delay' code path to burn the remaining cycles.

                ; The period-9 cycle-burning loop is written in a convoluted way to ensure that the loop
                ;   takes 9 cycles both in cases where a borrow happens, and in cases where it doesn't.
                ;
...
loop9_short:    sbc     #(9 - 1)    ; [2]   Subtract 9 from the cycle count. (carry is 0, here).
                bcs     loop9_long  ; [2/3] No borrow occurred. Make a long loop to burn 4 cycles and set carry to 0.
                dex                 ; [2]   Borrow occurred; we need to decrement X.
                bne     loop9_short ; [2/3] If X has become zero, we're done with this loop.

                ; Now X=0 and A has a number of cycles to burn. A is a "high" value (at least 256 - 9).

                ; Compensate for the overhead in the 'long_delay' code path.
                ;
                ; Subtracted 39 from the A register to ensure that the entire 'cycle_delay' routine consumes
                ...
                bcs     delay_a_reg ; [4] Branches across a page boundary.
